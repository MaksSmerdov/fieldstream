...
/**
 * Ограничитель на таблице, а не в памяти процесса: перезапуск шлюза не обнуляет счётчик попыток,
 * а при нескольких экземплярах ведро остаётся общим. Платят только неудачные попытки.
 */
