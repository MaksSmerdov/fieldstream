...
/**
 * Коды объектов это стабильные бизнес-идентификаторы, а не суррогатные id из БД.
 * Именно они уходят в ключ партиции Kafka: пересоздание базы иначе переразбросало бы
 * устройства по партициям и сломало порядок внутри прибора.
 */
