...
> **Статус: в разработке.** Готовы пакеты контрактов, кодека регистров, доменной логики и профилей приборов, симулятор приборов и базовая инфраструктура compose.
...
pnpm stack:up         # Kafka, TimescaleDB, топики и device-sim в Docker
```
...
Для ежедневной разработки без пересборки образов `pnpm infra:up` поднимает только брокер и базу, а сервис запускается на хосте: `pnpm --filter @fieldstream/device-sim dev`.
