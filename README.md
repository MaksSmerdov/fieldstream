...
Экран **Конвейер** показывает Kafka изнутри: группы потребителей с раскладкой партиций, отставание
по каждой партиции, журнал ребалансов и очередь недоставленных с повторной подачей. Экран **Отказы**
ломает стенд из интерфейса и показывает, как отвечает защита сборщика: размыкатель с отсчётом до
пробы, лестница переподключения по фактическим задержкам, сторож обхода и гистограмма времени
ответа с рекомендуемым таймаутом.
...
| Кто                          | Что может                                                                                      |
| ---------------------------- | ---------------------------------------------------------------------------------------------- |
| `viewer@fieldstream.local`   | только чтение, включая экраны Конвейер и Отказы                                                |
| `engineer@fieldstream.local` | подтверждение алармов, уставки, команды, поломки стенда, сценарии, повторная подача из очереди |
...
services/
  ...
  stream-processor/ разбор кадров, запись в TimescaleDB, алармы, здоровье, повторная подача из очереди
  api-gateway/      авторизация и права, REST, живой канал, команды, снимок конвейера, сценарии
