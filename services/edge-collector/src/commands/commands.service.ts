...
/**
 * Команды операторов. Все четыре меняют режим опроса линии, а не пишут в регистры прибора,
 * поэтому повторное применение безвредно, и защиты сложнее проверки срока здесь не нужно.
 * Ответ уходит обратно в брокер: про базу сборщик не знает, он стоит за NAT.
 */
@Injectable()
