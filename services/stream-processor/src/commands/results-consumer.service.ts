...
/**
 * Ответы исполнителей команд. Сборщик стоит за NAT и про базу не знает, поэтому факт
 * применения едет через брокер, а в таблицу его переносит процессор: он и так единственный,
 * кто пишет в схему. Повтор сообщения ничего не меняет, ключ идемпотентности это команда.
 */
@Injectable()
