...
/**
 * Приборы по коду: шлюзу они нужны не для записи, а для ключей подписки живого канала.
 * По прибору из события находятся его линия и площадка, поэтому вкладка может подписаться
 * на площадку, не перечисляя двадцать четыре кода.
 */
@Injectable()
