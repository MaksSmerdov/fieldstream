 ...
 * после обрыва добираются по последнему идентификатору, а сторож тишины переоткрывает
 * соединение, если сервер молчит дольше трёх keepalive. Кадр `resync` перезапрашивает всё.
 */
